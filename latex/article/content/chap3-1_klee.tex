\section{FEATURE DESCRIPTION}\label{chap:symbolic_execution_tools}

This section focuses on describing KLEE and Owi set of features (subsections \ref{section:klee} and \ref{section:owi}), finishing with a comparison between both tools, presented in \cref{section:feat_comp}.
...
% KLEE is in both static and dynamic techniques of testing: every symbolic execution engine is static by definition, but KLEE becomes dynamic through its outside environment interaction features \cite{Cadar2008}.
...
KLEE is packaged with multiple \glsxtrshort{cli}s that contribute to the user experience, but the main command that runs the symbolic execution engine is \lstinline{klee}. Another relevant command is \lstinline$ktest-tool$, which describes the generated test cases (\lstinline$.ktest$ files) in a human-readable format.
...
The KLEE web page contains a tutorials section to help get familiarized with the tool. These tutorials contain samples and instructions on how to properly set up a C/C++ source file, let's call it code instrumentation, and how to run and analyse the results \cite{klee_tutorial1,klee_tutorial2}. %Their sources can be found in higher detail in appendix \ref{appendix:klee}.

The first, and most basic example, shows how one can mark a variable as symbolic by using the \lstinline$klee_make_symbolic()$ function 
%(see \Cref{lst:klee_instrumentation}).
This function takes 3 arguments; the first two will define a memory range in which the variable information is stored, the third defines a human-readable object name that identifies the symbolic variable in the generated test case files. %(for the variable `\lstinline$a$' declared on \cref{var:klee_instrumentation_var_a} in \Cref{lst:klee_instrumentation}, this could be ``my variable''.):
...
% After instrumenting the source code with KLEE instructions, 
% % see \cref{subsection:klee_instrumentation}, 
% the next step is compiling it to LLVM.
% % , and the easiest way to achieve that is using Clang (\Cref{lst:klee_clang}).
% The KLEE documentation recommends the command in \Cref{lst:clang-complete} in order to have access to a more complete set of functionalities that klee provides, such as test replayability.
...
% ]
% clang -I $KLEE_LIBRARY_PATH -emit-llvm -c main.c
% \end{lstlisting}
