\section{Feature Comparison}\label{section:feat_comp}

Not only it is important to understand how well a tool performs, but also how flexible it can be in the different scenarios it may face. \Cref{tab:feature_comparison} was built describing each tool supported vulnerabilities, their ease-of-use, how easy they are to install, and which STP solvers they support. The tools' installation speed and complexity are scaled from 1-5 ranging from very slow to very fast or very hard to very easy, respectively. The third and fourth columns (Ease-of-use for beginner and experienced developers) take the same scale as the second one: 1-5, very hard to very easy.

The tools' installation process is very different both in the amount of methods but also in the complexity of the provided solutions. Although the recomended solution by \citeauthor{klee_install}\footfullcite{klee_install} is very complex since it requires a lot of dependencies and different processes, as we discussed in \cref{section:klee_install}, there is also available a quick start option using a docker image made available by the maintainers of KLEE. In contrast, Owi does not offer many ways to install its engine but both of them require the installation of \lstinline$opam$ but also \lstinline$dune$ if one compiles the source code directly. 
...
The last section of \Cref{tab:feature_comparison} aggregates information from KLEE and Owi documentation on SAT and SMT solvers. While SAT solvers handle \textit{boolean satisfiability}, SMT expand on this concept into general formulas\footfullcite{rbc_sat_solvers}. Examples of SMT solvers are STP and Z3 but there are also wrappers like MetaSMT and frontends as Smt.ml\footfullcite{stp_solver}\textsuperscript{,}\footfullcite{z3_solver}\textsuperscript{,}\footfullcite{metaSMT}\textsuperscript{,}\footfullcite{smtml}. MiniSat is a SAT solver used by STP as the underlying SAT solver and Smt.ml also has MiniSat in its support roadmap\footfullcite{minisat}\textsuperscript{,}\footnotemark[21]%smtml
.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{l|c|c}
       ...
       \hline
        ...
        Supports E-ACSL & \textcolor{red}{NO} & \textcolor{green}{YES} \\
        \hline
        \hline
        Supports MetaSMT & \textcolor{green}{YES} & \textcolor{red}{NO} \\
        Supports STP & \textcolor{green}{YES} & \textcolor{red}{NO} \\
        Supports Z3 & \textcolor{green}{YES} & \textcolor{green}{YES} \\
        Supports metaSMT solvers & \textcolor{green}{YES} & \textcolor{red}{NO} \\
        Supports Smt.ml solvers & \textcolor{red}{NO} & \textcolor{green}{YES} \\
        Supports MiniSat & \textcolor{green}{YES} & \textcolor{red}{NO}\\
    \end{tabular}
    \caption{Feature comparison}
    \label{tab:feature_comparison}
\end{table}

% \footnotetext[3]{`metaSMT is a wrapper around various SMT or SMT-like solvers.'\\\citep{metaSMT}}
% \footnotetext[4]{`Smt.ml is a Multi Back-end Front-end for SMT Solvers in OCaml.'\\\citep{smtml}}
