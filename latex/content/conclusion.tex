...
The metrics collected helped conclude that KLEE was overall superior both in performance and in its bug-finding capabilities. Despite being superior at finding vulnerabilities, KLEE still scores poorly in the OWASP Benchmark scoring table (54\%)\footfullcite{owasp_benchmark_scoring}, mainly due to its False Positive Rate, but the real culprit of this result is the size of the test sample.
...
\begin{itemize}
	...
	\Cref{equ:klee_youden_index,equ:owi_youden_index} show better results in favour of KLEE despite the small size of the test sample. According to \citet{owasp_benchmark_scoring}\footfullcite{owasp_benchmark_scoring}, KLEE's score of 54\% is considered as good as random guessing and Owi's 29\% mean nearly nothing is considered vulnerable code. Again, these results are a reflection of the small size of the test cases.
\end{itemize}
